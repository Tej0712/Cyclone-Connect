...
public class ReportChat {
    ...
    private final Logger logger = LoggerFactory.getLogger(ReportChat.class);
    ...
    public void onOpen(Session session, @PathParam("userId") Long userId) throws IOException {
        ...
        if (userIdSessionMap.containsKey(userId)) {
            ...
            sendMessageToParticularUser(userId, "Welcome to the chat server!");
    ...
    public void onMessage(Session session, String message) throws IOException {
        ...
        logger.info("[onMessage] " + userId + ": " + message);

        if (message.startsWith("@")) {
            // Direct messaging logic, assuming "@userId message"
            // This part would need adjustment for direct messaging based on userId
        } else {
